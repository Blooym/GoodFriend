...
internal sealed class ModuleService : IDisposable
{
    ...
    ///     All loaded modules.
    /// </summary>
    private readonly IEnumerable<BaseModule> loadedModules;
    ...
    /// </summary>
    /// <returns></returns>
    private static List<BaseModule> LoadModules()
    {
        ...
        foreach (var module in typeof(ModuleService).Assembly.GetTypes())
        {
            try
            {
                if (module.IsAbstract || !typeof(BaseModule).IsAssignableFrom(module))
                {
                    continue;
                }
    ...
    public IEnumerable<BaseModule> GetModules() => this.loadedModules;
}
